...
from LOSS import SSIM_LOSS, TV_LOSS, Fro_LOSS, _tf_fspecial_gauss
...
def train(source_imgs, save_path, EPOCHES_set, BATCH_SIZE, logging_period = 1):
	...
	with tf.Graph().as_default(), tf.Session() as sess:
		...
		print('generated img shape:', generated_img.shape)
		...
		D1_real = D1.discrim(grad(SOURCE_VIS), reuse = False)
		D1_fake = D1.discrim(grad(generated_img), reuse = True)
		...
		LOSS_VIS = TV_LOSS(grad(generated_img) - grad_of_vis)
		G_loss_norm = LOSS_IR + 0.8 * LOSS_VIS
		G_loss = G_loss_GAN + 3 * G_loss_norm
		...
		tf.summary.image('ir', SOURCE_ir, max_outputs = 4)
		tf.summary.image('vis', SOURCE_VIS, max_outputs = 4)
		tf.summary.image('fused_result', generated_img, max_outputs = 4)
		...
		for epoch in range(EPOCHS):
			...
			for batch in range(n_batches):
				...
				while (d1_loss < 1.2 or d2_loss < 1.2) and it_g < 20:  # or _loss < 1) and it_g < 20:
				...
				print("Epoch:%s: batch:%s,  D1:%s D2:%s, G:%s" % (epoch + 1, batch, it_d1, it_d2, it_g))
				...
				if is_last_step or step % logging_period == 0:
					...
					g_loss_adv, g_loss_norm = sess.run([G_loss_GAN, G_loss_norm], feed_dict = FEED_DICT)
					...
					print('G_loss_gan: %s, G_loss_norm: %s' % (g_loss_adv, g_loss_norm))
					maxv = np.max(g_img)
					minv = np.min(g_img)
